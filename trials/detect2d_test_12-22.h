                   ...
                   std::string basePath, std::string classesFile, std::string modelConfiguration, std::string modelWeights, bool bVis)
{
    ...
    if(bVis) 
    {
        ...
        for(auto it=bBoxes.begin(); it!=bBoxes.end(); ++it) {
            ...
            cv::rectangle(visImg, cv::Point(left, top), cv::Point(left + width, top + height), cv::Scalar(get<1>(RGB_Color), get<2>(RGB_Color), get<0>(RGB_Color)), 2);
            ...
            //cv::Size labelSize = getTextSize(label, cv::FONT_ITALIC, 1, 2, &baseLine);//best
            cv::Size labelSize = getTextSize(label, cv::FONT_ITALIC, .75, 2, &baseLine);//best
            ...
            //cv::putText(visImg, label, cv::Point(left, top - round(0.35 * labelSize.height)), cv::FONT_ITALIC, 1, cv::Scalar(0, 0, 0), 2);//best
        ...
        if(bBoxes.size()>0)
            imwrite(output_path + std::to_string(f) + ".png", visImg);
        
        //cv::Mat visImgshow = visImg.clone();    //new
        
        string windowName = "Object classification";
        //cv::namedWindow( windowName, 1 );
        cv::namedWindow(windowName, cv::WINDOW_NORMAL);
        cv::imshow( windowName, visImg );
        //cv::imshow(windowName, visImgshow);     //new
